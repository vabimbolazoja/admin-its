...
const DepositTransactions = (props) => {
  ...
  const searchReconciliationFunc = () => {
    ...
    setLoad(true)
      axios
        .get(
          `${config.baseUrl}/api/v1/admin/transactions/wallet?pageNumber=1&pageSize=10&amount=${amount}&provider=${provider}&transactionType=${transactionType}&startDate=${startDate}&endDate=${endDate}&currency=${currency}&transactionStatus=${transactionStatus}&emailAddress=${emailAddress}&transactionReference=${transRef}`,
          {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            },
  ...
  const pagination = (page, pageSize) => {
    setPage(page);
    const queryString = `pageNumber=${page}&pageSize=${pageSize}&amount=${amount}&provider=${provider}&transactionType=${transactionType}&startDate=${startDate}&endDate=${endDate}&currency=${currency}&transactionStatus=${transactionStatus}&emailAddress=${userEmail}`;
  ...
  const getReconciliations = () => {
    axios
      .get(
        `${config.baseUrl}/api/v1/admin/transactions/wallet?pageNumber=1&pageSize=100&amount&provider&transactionType&startDate&endDate&currency&transactionStatus&emailAddress=${userEmail}`,
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
          },
