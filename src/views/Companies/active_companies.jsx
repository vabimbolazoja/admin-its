...
const Users = () => {
  ...
  const searchUsersFilter = (e) => {
    e.preventDefault();
    setLoad(true);
    const queryString = `pageNumber=${page}&pageSize=10&name=${name}&status=ACTIVE`;
  ...
  const pagination = (page, pageSize) => {
    console.log(page);
    setPage(page);
    const queryString = `pageNumber=${page}&pageSize=${pageSize}&name&status=ACTIVE`;
    getKycPaged(queryString);
  };
  ...
  const getUsersKyc = () => {
    axios
      .get(
        `${config.baseUrl}/api/v1/admin/companies?pageNumber=1&pageSize=100&name&status=ACTIVE`,
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
          },
                ...
                Actions: (item) => (
                  <td className="d-flex">
                    <button
                      ...
                      onClick={changeStatusDecline.bind(this, item.id)}
                    >
                      Deactivate
                    </button>
                    <button
