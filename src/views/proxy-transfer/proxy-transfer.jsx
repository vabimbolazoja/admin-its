...
const ProxyTransfer = (props) => {
  ...
  const [emailToCheck, setEmailToCheck] = useState("");
    const [openInitiateBankProxy, setOpenInitiateBankProxy] = useState(false);
  const [configsData, setConfigsData] = useState({});
  const [bankProxy, setBankProxy] = useState(false)
  ...
  const closeInitiateTransfer = () => {
    ...
    const closeBankInitiateTransfer = () => {
    setOpenInitiateBankProxy(false);
    setCurrency("");
    setAmount("");
    setReason("");
    setCutomFee("");
  ...
  const pagination = (page, pageSize) => {
    setPage(page);
    const queryString = `pageNumber=${page}&pageSize=${pageSize}&amount=${amount}&proxyInitiator=&startDate=${startDate}&endDate=${endDate}&currency=${currency}&proxyTransferReason=${reason}&transactionStatus=${transactionStatus}`;
  ...
  `,
          {
            ...
            expense: isExpense,
            refund: refund,
                businessProxyTransfer: bankProxy
  ...
  `,
          {
            ...
            currency: currency,
            reason: reason,
            selfAppraisedFee: true,
            instantProxy: instantProxy,
    businessProxyTransfer: true
        ...
        .then((res) => {
          ...
          if (res.status === 200) {
            ...
            setCutomFee("");
            setFeeModal(false);
            setMsg("Bank Proxy Transfer Initiated Successfully");
            setTimeout(() => {
              setMsg("");
              setSuccess(false);
            }, 2500);
  ...
  const getProxyTransfers = () => {
    axios
      .get(
        `${config.baseUrl}/api/v1/admin/transactions/proxy-transfer?pageNumber=1&pageSize=100`,
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
          },
  ...
  const searchProxyFunc = () => {
    ...
    axios
      .get(
        `${config.baseUrl}/api/v1/admin/transactions/proxy-transfer?pageNumber=1&pageSize=100&amount=${amount}&proxyInitiator=&startDate=${startDate}&endDate=${endDate}&currency=${currency}&proxyTransferReason=${reason}&transactionStatus=${transactionStatus}&beneficiaryEmail=${beneficiaryEmail}&senderEmail=${senderEmail}`,
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
          },
  ...
  return (
    <CRow>
        ...
        <CCard>
          <CCardHeader>
            <div className="d-flex justify-content-end align-items-center mb-2">
                <Button type="success" onClick={() => setOpenInitiateBankProxy(true)} className="mr-2">
                 Bank Account Refund
              </Button>
              <ExportCSV
          ...
          <CFormGroup>
            <div class="mb-3 form-check">
              <input
                ...
                class="form-check-input"
                id="exampleCheck1"
                onChange={(e) => setBankProxy(e.target.checked)}
                checked={bankProxy}
              />
              <label class="form-check-label" for="exampleCheck1">
                {" "}
      ...
      <Modal
        title={"Initiate Bank Account Refund"}
        visible={openInitiateBankProxy}
        footer={null}
        maskClosable={false}
        onCancel={closeBankInitiateTransfer}
      >
        <div>
          {success && <CAlert color="success">{msg}</CAlert>}
